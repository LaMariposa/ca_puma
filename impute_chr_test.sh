...
targetlist=/scratch3/puma_megan/impute/inputfiles/lc_ds_359.list
...
${shapeit}/phase_common_static --input refpanel_${chr}.vcf.gz --region ${chr} --output refpanel_${chr}.sphased.bcf --thread ${threads} --map puma_poschcm.map
...
java -jar ${beagle} gt=refpanel_${chr}.vcf.gz out=refpanel_${chr}.bphased nthreads=${threads} map=puma_plink.map window=0.400 overlap=0.020
	#could specify ne
...
OUT=s_${chr}_${ID}.bcf
${glimpse}/GLIMPSE_phase_static --input imppanel_${chr}.vcf.gz --reference refpanel_${chr}.sphased.bcf --input-region ${IRG} --output-region ${ORG} --output ${OUT} --thread ${threads} --map puma_poschcm.map	
...
ORG=$(echo $LINE | cut -d" " -f4)
OUT=b_${chr}_${ID}.bcf
${glimpse}/GLIMPSE_phase_static --input imppanel_${chr}.vcf.gz --reference refpanel_${chr}.bphased.vcf.gz --input-region ${IRG} --output-region ${ORG} --output ${OUT} --thread ${threads} --map puma_poschcm.map
bcftools index -f ${OUT}
